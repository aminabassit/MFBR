...
const int NFEAT = 512;
const size_t COMPLEN = 926; // number of scores between the threshold to Smax
...
int main(int argc, char *argv[]) {
    ...
    string sBits = argv[1];
    string RUNTIMEFILE = "../experiments/runtimesMFIP/MFIPEncComp/expMFIP-EncComp"+sBits+".csv";
    string TEMPRUNTIMEFILE = "../experiments/runtimesMFIP/MFIPEncComp/expMFIP-EncComp-Template"+sBits+".csv";
    string PROBRUNTIMEFILE = "../experiments/runtimesMFIP/MFIPEncComp/expMFIP-EncComp-Probe"+sBits+".csv";
    ...
    Plaintext onePlain = cryptoContext->MakePackedPlaintext({1,0});
    ...
    for (int subject1 = 1; subject1 < SUBJECTS+1; subject1++) {
        ...
        */
        ...
        TIC(runVerif);
        auto finalScoreCt0 = computeFinalScoreForBlindedCompGroups(cryptoContext, subj1RefTempCt, permProbInd, onePlain);
        ...
        {
            finalScoreCt0 = cryptoContext->EvalAtIndex(finalScoreCT, pow(2,i)); 
        ...
        cryptoContext->EvalAddInPlace(finalScoreCT, compVectCT);
        finalScoreCT = cryptoContext->EvalMult(finalScoreCT, blindRCT);    
        
        Plaintext finalScorePT;
        cryptoContext->Decrypt(keyPair.secretKey, finalScoreCT, &finalScorePT);      
        auto res = verifyComparison(finalScorePT, COMPLEN);  
